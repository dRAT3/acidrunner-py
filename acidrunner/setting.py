...
class Settings:
    ...
    def from_dict(cls, config_dict: Dict) -> "Settings":
        return cls(
            systems_under_test=cls._parse_systems_under_test(
                config_dict.get("systems_under_test", [])
            ),
            buckets=cls._parse_buckets(config_dict.get("buckets", [])),
            file_settings=cls._parse_file_settings(
                config_dict.get("file_settings", {})
            ),
        )
    ...
    def _parse_systems_under_test(systems_list: List[Dict]) -> List[SystemUnderTest]:
        return [
            ...
            if "name" in system and "entrypoint" in system
        ]

    @staticmethod
    def _parse_buckets(buckets_list: List[Dict]) -> List[Bucket]:
        return [
            Bucket(name=bucket["name"], rpm=bucket["rpm"])
            for bucket in buckets_list
            if "name" in bucket and "rpm" in bucket
        ]

    @staticmethod
    def _parse_file_settings(file_settings: Dict) -> FileSettings:
        return FileSettings(
            data_dir=file_settings[0]["data_dir"], sqlite_enable=file_settings[1]
        )
    ...
    def __repr__(self):
        return (
            f"Settings(systems_under_test={self.systems_under_test}, "
